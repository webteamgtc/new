...
export async function generateMetadata({ params, searchParams }, parent) {
  ...
  const url =
    locale != "en"
      ? `${process.env.NEXT_PUBLIC_BASE_URL}/${locale}/${"latest-news"}/${params?.slug}`
      : `${process.env.NEXT_PUBLIC_BASE_URL}/${"latest-news"}/${params?.slug}`;
  ...
  return {
    ...
    openGraph: {
      ...
      title: product?.data?.[0]?.attributes?.title,
      description: product?.data?.[0]?.attributes?.short_descreption,
      url: `https://www.gtcfx.com/${"latest-news"}/${params?.slug}`,
    },
    alternates: {
      canonical: url,
    },
